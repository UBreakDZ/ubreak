...
function StoreProvider({ children }) {
  ...
  const loadUserData = async (uid) => {
    try {
      const { data: adminData } = await supabase.from("admin_users").select("id").eq("id", uid).maybeSingle();
  ...
  const signIn = async (cardNumber, password) => {
    try {
      const email = `${cardNumber}@ubreak.dz`;
      const { error } = await supabase.auth.signInWithPassword({ email, password });
      if (error) return { error: "Numéro de carte ou mot de passe incorrect." };
      return { ok: true };
  ...
  const signUp = async (cardNumber, firstName, lastName, password) => {
    try {
      ...
      return { ok: true };
    } catch(e) { return { error: "Erreur lors de l'inscription." }; }
  };
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
      <div style={{background:`linear-gradient(160deg,${C.greenDark},${C.green})`,padding:"40px 28px 32px",borderBottomLeftRadius:32,borderBottomRightRadius:32}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:44,height:44,background:"rgba(255,255,255,.2)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>🍽️</div>
...
function RegisterScreen({ onGoLogin }) {
  ...
  const handle=async()=>{
    ...
    if(form.pwd.length<6){setErr("Mot de passe minimum 6 caractères.");return;}
    setBusy(true); setErr("");
    const res = await signUp(form.card, form.firstName, form.lastName, form.pwd);
    if(res.error){setErr(res.error); setBusy(false);}
    else { setSuccess(true); setBusy(false); }
  };
  if(success) return (
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32,minHeight:"100vh",background:C.cream}}>
      <div style={{fontSize:64,marginBottom:16}}>🎉</div>
      <div style={{fontWeight:900,fontSize:22,color:C.text,marginBottom:8,textAlign:"center"}}>Compte créé !</div>
      <div style={{fontSize:15,color:C.textMuted,textAlign:"center",marginBottom:32}}>Tu peux maintenant te connecter.</div>
      <Btn onClick={onGoLogin}>Se connecter</Btn>
    </div>
  );
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
      ...
      <div style={{flex:1,padding:"28px 24px"}}>
        ...
        <Input label="Mot de passe" type="password" value={form.pwd} onChange={set("pwd")} placeholder="6 caractères minimum"/>
...
function MenuScreen({ cart, setCart, onCheckout, onLogout }) {
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
      ...
      {cartCount>0&&(
        <div onClick={onCheckout} style={{position:"fixed",bottom:20,left:16,right:16,background:C.green,borderRadius:18,padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:"0 8px 24px rgba(78,154,14,.45)",cursor:"pointer",zIndex:50}}>
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
      ...
      <div style={{flex:1,padding:"20px 20px 30px"}}>
        ...
        {status===2&&(
          <div style={{background:`linear-gradient(135deg,${C.green},${C.greenLight})`,borderRadius:20,padding:24,textAlign:"center",color:"#fff"}}>
...
const ADMIN_NAV=[{id:"dashboard",icon:"📊",label:"Dashboard"},{id:"orders",icon:"🧾",label:"Commandes"},{id:"menu",icon:"🍽️",label:"Menu"},{id:"ingredients",icon:"🥬",label:"Ingrédients"},{id:"stock",icon:"📦",label:"Stocks"}];
...
function AdminApp({ onLogout }) {
  ...
  const renderSection=()=>{
    ...
    return (
      <div>
        <h1 style={{fontWeight:900,fontSize:22,color:C.text,marginBottom:4}}>Tableau de bord</h1>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:C.cream,fontFamily:"'Nunito','Segoe UI',sans-serif"}}>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:C.sidebar,display:"flex",justifyContent:"space-around",padding:"8px 0 12px",zIndex:50}}>
        {ADMIN_NAV.map(item=>(
...
function AdminMenu() {
  ...
  const save=async()=>{ if(!form.name||!form.price)return; setBusy(true); if(modal==="add") await addMenuItem({...form,price:Number(form.price)}); else await updateMenuItem({...form,price:Number(form.price)}); setBusy(false); setModal(null); await loadMenu?.(); };
  const doToggle=async(item)=>{ await toggleMenuItem(item.id,!item.active); };
  return (
    <div>
      ...
      {menu.map((item)=>(
        <div key={item.id} style={{background:C.white,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:item.active?C.success:C.danger,fontWeight:700,marginTop:2}}>{item.active?"● Actif":"● Désactivé"}</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <Toggle value={item.active} onChange={()=>doToggle(item)}/>
...
function ClientApp({ onAdminRequest }) {
  const { profile, placeOrder, signOut, loading, session } = useStore();
  ...
  useEffect(()=>{
    if(!loading){
      if(profile) setScreen("menu");
      else if(!session) setScreen(prev => prev==="splash" ? "splash" : "login");
    }
  ...
  if(loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:C.cream}}>
      <div style={{fontSize:16,color:C.textMuted,fontWeight:700}}>⏳ Chargement...</div>
    </div>
  );

  return (
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:C.cream,fontFamily:"'Nunito','Segoe UI',sans-serif",position:"relative"}}>
...
function AppRouter({ showAdminLogin, setShowAdminLogin }) {
  ...
  if(loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#1A2E0A"}}>
      <div style={{color:"#fff",fontSize:16,fontWeight:700}}>⏳ Connexion à U'Break...</div>
    </div>
  );
